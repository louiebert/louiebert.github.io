...
- **Before running `npm run dev`**: Always check if port 3000 is in use with `netstat -an | findstr :3000`
  - **Rule**: Do NOT run `npm run dev` if localhost:3000 is already in use (indicates existing dev server)
